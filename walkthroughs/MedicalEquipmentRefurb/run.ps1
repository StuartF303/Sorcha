...
#
# MedicalEquipmentRefurb — Run
# Execute 3 scenarios: A (routine), B (safety-critical), C (rejection).
...
Write-WtBanner "MedicalEquipmentRefurb — Run"
...
# Convert wallets PSObject to hashtable
$wallets = @{}
foreach ($prop in $state.wallets.PSObject.Properties) { $wallets[$prop.Name] = $prop.Value }

# Action-to-sender mapping for medical equipment (action IDs are 1-5)
$actionSenderMap = @{
...
$scenarioFiles = @{
    'A' = "data/scenario-a-routine.json"
    'B' = "data/scenario-b-safety-critical.json"
...
Write-Host ""
Write-WtBanner "MedicalEquipmentRefurb — Results"
