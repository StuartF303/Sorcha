...
public class DashboardLayoutTests : PageTest
{
    ...
    private async Task<bool> LoginAsync()
    {
        try
        {
            ...
            try
            {
                ...
                TestContext.WriteLine("Blazor loaded");
            }
            catch
            {
                TestContext.WriteLine("Warning: Blazor object not found, continuing anyway");
            }

            // Additional wait for content to render
            await Page.WaitForTimeoutAsync(10000);
            TestContext.WriteLine("Waited 10s for rendering");
            ...
            if (_screenshotDir != null)
            {
                ...
                TestContext.WriteLine($"Login page screenshot: {loginScreenshot}");
            }
            ...
            try
            {
                ...
                TestContext.WriteLine("Found username input");
            }
            catch
            {
                TestContext.WriteLine("Username input not found with type='text', trying placeholder");
            ...
            try
            {
                ...
                TestContext.WriteLine("Found password input");
            }
            catch
            {
                TestContext.WriteLine("Password input not found");
                throw;
            }
            ...
            TestContext.WriteLine($"Filled credentials: {TestEmail}");
            ...
            TestContext.WriteLine($"Found {profileCount} select elements");
            ...
            if (profileCount > 0)
            {
                try
                {
                    ...
                    TestContext.WriteLine($"Available profiles: {string.Join(", ", options)}");
                    ...
                    if (dockerOption != null)
                    {
                        ...
                        TestContext.WriteLine($"Selected profile: {dockerOption}");
                    }
                    else
                    {
                        TestContext.WriteLine("docker profile not found, using default");
                    }
                }
                catch (Exception ex)
                {
                    TestContext.WriteLine($"Profile selection warning: {ex.Message}");
                }
            }
            ...
            if (errorCount > 0)
            {
                for (int i = 0; i < errorCount; i++)
                {
                    ...
                    TestContext.WriteLine($"Error message {i + 1}: {errorText}");
                }
            }
            ...
            if (spinnerVisible)
            {
                TestContext.WriteLine("Login spinner still visible, waiting longer...");
                await Page.WaitForTimeoutAsync(10000);
                TestContext.WriteLine($"After extended wait, URL: {Page.Url}");
            }

            // Check if login succeeded
            var currentUrl = Page.Url;
            var loginSucceeded = !currentUrl.Contains("/auth/login");
            TestContext.WriteLine($"Login succeeded: {loginSucceeded}");
            ...
            if (!loginSucceeded && _screenshotDir != null)
            {
                ...
                TestContext.WriteLine($"Login failed screenshot: {failedScreenshot}");
            }
        ...
        {
            TestContext.WriteLine($"Login failed with exception: {ex.Message}");
            ...
            if (_screenshotDir != null)
            {
                ...
                TestContext.WriteLine($"Error screenshot: {errorScreenshot}");
            }
    ...
    public async Task Dashboard_LoadsAfterLogin()
    {
        ...
        TestContext.WriteLine($"Page title: {await Page.TitleAsync()}");
    }
    ...
    public async Task Dashboard_LayoutInspection()
    {
        ...
        TestContext.WriteLine($"Full page screenshot: {fullScreenshotPath}");

        // Get page dimensions and layout info
        var viewport = Page.ViewportSize;
        TestContext.WriteLine($"Viewport: {viewport?.Width}x{viewport?.Height}");
        ...
        foreach (var element in elements)
        {
            ...
            TestContext.WriteLine($"{element.Key}: Found={count}, Visible={isVisible}");
            ...
            if (isVisible)
            {
                ...
                if (box != null)
                {
                    TestContext.WriteLine($"  Position: ({box.X}, {box.Y}), Size: {box.Width}x{box.Height}");
                }
            }
        }
        ...
        TestContext.WriteLine($"MudBlazor components found: {mudCount}");
        ...
        {
            TestContext.WriteLine("WARNING: Possible overlapping elements detected!");
        }
    ...
    public async Task Dashboard_ResponsiveLayout_Desktop()
    {
        ...
        TestContext.WriteLine($"Desktop screenshot: {desktopScreenshot}");
    ...
    public async Task Dashboard_ResponsiveLayout_Tablet()
    {
        ...
        TestContext.WriteLine($"Tablet screenshot: {tabletScreenshot}");
    ...
    public async Task Dashboard_ResponsiveLayout_Mobile()
    {
        ...
        TestContext.WriteLine($"Mobile screenshot: {mobileScreenshot}");
    ...
    public async Task Dashboard_CSSAnalysis()
    {
        ...
        TestContext.WriteLine("CSS Analysis Results:");
        TestContext.WriteLine(System.Text.Json.JsonSerializer.Serialize(cssIssues, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
