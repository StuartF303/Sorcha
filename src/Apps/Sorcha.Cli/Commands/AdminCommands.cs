...
public class AdminCommand : Command
{
        ...
        : base("admin", "Administrative operations")
    {
        ...
        Subcommands.Add(new AdminSchemaSectorsCommand(clientFactory, authService, configService));
        Subcommands.Add(new AdminSchemaProvidersCommand(clientFactory, authService, configService));
...
/// </summary>
public class AdminSchemaSectorsCommand : Command
{
    public AdminSchemaSectorsCommand(
        HttpClientFactory clientFactory,
        IAuthenticationService authService,
        IConfigurationService configService)
        : base("schema-sectors", "List schema sectors")
    {
        this.SetAction(async (ParseResult parseResult, CancellationToken ct) =>
        {
            try
            {
                ...
                var client = await clientFactory.CreateAdminServiceClientAsync(profileName);
                var sectors = await client.ListSchemaSectorsAsync($"Bearer {token}");

                var outputFormat = parseResult.GetValue(BaseCommand.OutputOption) ?? "table";
                if (outputFormat.Equals("json", StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine(JsonSerializer.Serialize(sectors, new JsonSerializerOptions { WriteIndented = true }));
                    return ExitCodes.Success;
                }

                if (sectors == null || sectors.Count == 0)
                {
                    ConsoleHelper.WriteInfo("No schema sectors found.");
                    return ExitCodes.Success;
                }

                ConsoleHelper.WriteSuccess($"Found {sectors.Count} schema sector(s):");
                Console.WriteLine();
                Console.WriteLine($"{"ID",-38} {"Name",-25} {"Schemas",7} {"Status",-10} {"Description"}");
                Console.WriteLine(new string('-', 110));

                foreach (var sector in sectors)
                {
                    Console.WriteLine($"{sector.Id,-38} {sector.Name,-25} {sector.SchemaCount,7} {sector.Status,-10} {sector.Description}");
                }
            ...
            {
                ConsoleHelper.WriteError($"Failed to list schema sectors: {ex.Message}");
                return ExitCodes.GeneralError;
            }
        });
    }
...
/// </summary>
public class AdminSchemaProvidersCommand : Command
{
    public AdminSchemaProvidersCommand(
        HttpClientFactory clientFactory,
        IAuthenticationService authService,
        IConfigurationService configService)
        : base("schema-providers", "List schema providers")
    {
        this.SetAction(async (ParseResult parseResult, CancellationToken ct) =>
        {
            try
            {
                ...
                var client = await clientFactory.CreateAdminServiceClientAsync(profileName);
                var providers = await client.ListSchemaProvidersAsync($"Bearer {token}");
                ...
                Console.WriteLine();
                Console.WriteLine($"{"ID",-38} {"Name",-25} {"Schemas",7} {"Status",-10} {"Base URL"}");
                Console.WriteLine(new string('-', 110));

                foreach (var provider in providers)
                {
                    Console.WriteLine($"{provider.Id,-38} {provider.Name,-25} {provider.SchemaCount,7} {provider.Status,-10} {provider.BaseUrl}");
                }
