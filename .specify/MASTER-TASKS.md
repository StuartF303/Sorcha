...
| 3 | Investigate validator initialization loop (no transaction processing observed) | ✅ Resolved: `ValidationEngineService` IS registered as hosted service via `AddValidationEngine()`. Pipeline works end-to-end (Construction Permit 11/11, Medical Equipment Refurb 12/12). Issue was pre-pipeline-rewire (2026-02-14). |
...
| 6 | Wire governance operations through validator pipeline | ✅ Governance submission endpoints: `POST .../governance/propose` (Add/Remove/Transfer as Control TX), `GET .../governance/proposals` (paginated history). Service client methods + DTOs. DocketSerializer now passes submission metadata to TrackingData for filtering. 7 new tests. |
| 7 | Clean up orphan transaction from prior direct-write (MongoDB) | ✅ Admin endpoints: `GET/DELETE /api/admin/registers/{id}/orphan-transactions`. Detects transactions not referenced by any docket, safety-checks chain integrity before deletion. `DeleteTransactionAsync` added to `IRegisterRepository` + all implementations. |
